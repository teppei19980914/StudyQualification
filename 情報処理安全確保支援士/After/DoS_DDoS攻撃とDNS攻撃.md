...
正常なHTTPリクエストを大量送信、またはアプリケーション処理を意図的に遅延させる攻撃です。
帯域消費が少ないため検知が困難な場合があります。
...
| OSI層 | 攻撃例 | 特徴 |
|------|------|------|
| L3(ネットワーク層) | ICMP Flood、Smurf、Ping of Death | IPレベルの大量パケット |
| L4(トランスポート層) | SYN Flood、UDP Flood、LAND攻撃 | TCP/UDPの仕様悪用 |
| L7(アプリケーション層) | Slowloris、HTTP Flood、DNS水責め | 正常通信に偽装、検知困難 |

L3/L4攻撃は帯域・接続リソース枯渇を狙い、L7攻撃はアプリケーションの処理能力枯渇を狙います。
**防御にはそれぞれ異なるアプローチが必要**であり、試験でも区別が問われます。

### SYN Flood(RFC 4987)

TCP接続の「SYNキュー」を枯渇させる古典的攻撃です。

#### 攻撃の仕組み(3ウェイハンドシェイクの悪用)

TCP接続は通常、以下の3ウェイハンドシェイクで確立されます。

```
クライアント → SYN → サーバ
クライアント ← SYN+ACK ← サーバ  (サーバはSYNキューにエントリを作成)
クライアント → ACK → サーバ      (接続確立、SYNキューから削除)
```

SYN Flood では、攻撃者が **送信元IPを偽装したSYNパケットを大量送信**します。

1. サーバは SYN+ACK を返すが、偽装IPのため ACK が返ってこない
2. サーバは **ハーフオープン接続(half-open connection)** としてSYNキューに保持し続ける
3. SYNキューが満杯になると、**正規ユーザの新規接続を受け付けられなくなる**

#### SYN Cookie(主要対策)

SYN Cookie は **SYNキューを使わずにTCP接続を管理する技術**です。

仕組み：

1. SYN受信時、SYNキューにエントリを作成しない
2. 代わりにSYN+ACKの **シーケンス番号に接続情報を暗号学的にエンコード**して返す
3. 正規クライアントからACKが返ると、シーケンス番号から接続情報を復元
4. SYNキューが枯渇しないため、正規接続を受付可能

**SYN Flood に対して最も効果的な対策**として試験で頻出です。
...
- SYNキューサイズの拡大
- ハーフオープン接続のタイムアウト短縮
- ISPでのIngress Filtering(送信元IP偽装の防止)

### LAND攻撃

LAND(Local Area Network Denial)攻撃とは、**送信元IPアドレスと宛先IPアドレスの両方を、攻撃対象のIPアドレスに偽装したパケットを送信する攻撃**です。

#### 仕組み

- 送信元IP = 宛先IP = 攻撃対象のIP
- 送信元ポート = 宛先ポート に設定する場合もある
- 攻撃対象は「自分自身への応答」を処理しようとしてループ状態に陥る

#### 対策

- OS/ファイアウォールで送信元と宛先が同一のパケットを破棄
- 現代のOSではほぼ対策済みだが、**過去問では出題実績あり**

### Connection Flood(TCP Full Connection Flood)

SYN Floodと異なり、**3ウェイハンドシェイクを正常に完了させた上で大量のTCPコネクションを張り続ける攻撃**です。

#### 特徴

- 正規の接続を大量に確立するため、**SYN Cookieでは防げない**
- サーバのコネクション数上限を枯渇させる
- 送信元IPを偽装しないため、IPスプーフィング対策も無効

#### 対策

- 同一IPからの同時接続数制限
- 接続タイムアウトの短縮
- WAF/ロードバランサによるコネクション管理
...
Fraggle攻撃とは、**Smurf攻撃のUDP版**です。

### 仕組み

- Smurf攻撃がICMP Echo(Ping)を使うのに対し、Fraggle攻撃は**UDPパケット(主にUDP Echo: ポート7)**を使用
- 送信元IPを被害者に偽装し、ブロードキャストアドレス宛にUDPパケットを送信
- 各ホストが被害者IPに対してUDP応答を返送し、帯域を枯渇させる

### Smurf攻撃との比較

| 項目 | Smurf攻撃 | Fraggle攻撃 |
|------|-----------|-------------|
| 使用プロトコル | ICMP | UDP |
| 送信先 | ブロードキャストアドレス | ブロードキャストアドレス |
| 増幅方法 | ICMP Echo Reply | UDP応答 |
| 対策 | directed broadcast禁止 | directed broadcast禁止 + 不要UDPサービス停止 |

### 対策

- Smurf攻撃と同様のdirected broadcast禁止
- 不要なUDPサービス(echo、chargen等)の停止
...
## Slow HTTP DoS攻撃(アプリケーション層DoS)

Slow HTTP DoS攻撃とは、**HTTPリクエストやレスポンスを意図的に低速で送信し続けることで、Webサーバのコネクションを長時間占有し、サービス不能状態を引き起こす攻撃**です。

特徴：

- 大量のトラフィックを必要としない(**少量の通信で攻撃可能**)
- 正常なHTTP通信に見えるため、**従来のDDoS対策では検知困難**
- L7(アプリケーション層)攻撃に分類される

### Slowloris(Slow HTTP Headers)

HTTPリクエストの**ヘッダ送信を完了させずに、不完全なヘッダを少しずつ送り続ける**攻撃です。

#### 仕組み

1. 攻撃者がWebサーバにHTTP接続を確立
2. HTTPリクエストヘッダを送信するが、**最後の空行(CRLF×2)を送らない**
3. 定期的にヘッダの一部を追加送信し、タイムアウトを回避
4. サーバはリクエスト完了を待ち続け、**コネクションが占有される**
5. 同時接続数上限に達すると、正規ユーザがアクセス不能

#### ポイント

- Apacheなど**接続完了を待つ型のWebサーバ**が特に脆弱
- 1台のPCからでも攻撃可能(低帯域攻撃)

### Slow HTTP POST(RUDY: R-U-Dead-Yet)

HTTPリクエストの**POSTボディを極めて低速に送信し続ける**攻撃です。

#### 仕組み

1. 正常なHTTPヘッダを送信(Content-Length に大きな値を指定)
2. POSTボディを**1バイトずつ、数秒間隔で送信**
3. サーバはContent-Length分のデータ受信完了を待ち続ける
4. コネクションが長時間占有される

### Slow Read DoS

**HTTPレスポンスの受信ウィンドウサイズを極端に小さくする**ことで、サーバ側のレスポンス送信を遅延させる攻撃です。

#### 仕組み

1. 正常なHTTPリクエストを送信
2. TCP受信ウィンドウサイズ(Window Size)を極小値(例：1バイト)に設定
3. サーバはレスポンスを送りたいが、**ウィンドウサイズが小さいため少しずつしか送れない**
4. レスポンス送信完了までコネクションが占有される

### Slow HTTP DoS攻撃の比較

| 攻撃名 | 低速化する対象 | 攻撃の段階 |
|------|------|------|
| Slowloris | リクエストヘッダ | リクエスト送信時 |
| Slow POST(RUDY) | リクエストボディ | リクエスト送信時 |
| Slow Read | レスポンス受信 | レスポンス受信時 |

### Slow HTTP DoS攻撃の対策

- **接続タイムアウトの短縮**(ヘッダ/ボディ受信の制限時間設定)
- **同一IPからの同時接続数制限**
- **最小転送速度の設定**(Apache: `RequestReadTimeout`、nginx: `client_body_timeout`)
- リバースプロキシ/WAFによるリクエスト検査
- **Webサーバの選定**：nginx等のイベント駆動型サーバはSlowlorisに対して耐性が高い

## HTTP Flood(L7 DDoS攻撃)

HTTP Floodとは、**大量の正常なHTTPリクエストをWebサーバに送信し、アプリケーション層の処理能力を枯渇させる攻撃**です。

### 特徴

- 各リクエストは**正常なHTTP通信**であり、プロトコル異常がない
- L3/L4レベルのDDoS対策(SYN Cookie等)では防げない
- ボットネットを利用して分散実行される
...
| 種類 | 説明 |
|------|------|
| HTTP GET Flood | 大量のGETリクエストでページ/画像等を要求 |
| HTTP POST Flood | フォーム送信等のPOSTリクエストを大量送信 |
| ランダムURL攻撃 | ランダムなURLを生成しキャッシュを無効化 |

### 対策

- WAF(Web Application Firewall)による異常パターン検出
- CAPTCHA/JavaScript Challengeによるボット排除
- レートリミットの導入
- CDNによるキャッシュ活用と負荷分散

## EDoS攻撃(Economic Denial of Sustainability)

EDoS攻撃とは、**クラウド環境のオートスケール機能を悪用し、攻撃対象に過大な従量課金を発生させることで経済的損害を与える攻撃**です。

### 特徴

- サービス自体は停止しない(オートスケールが機能するため)
- **経済的な持続可能性(Sustainability)を破壊**する
- クラウド環境特有の脅威

### 対策

- クラウドリソースの**課金上限(Budget Alert)設定**
- オートスケールの**上限値設定**
- 異常トラフィックの早期検知と遮断
- WAF/CDNによるボットトラフィック排除
...
| 技術 | ポート | 特徴 |
|------|------|------|
| DoT(RFC 7858) | TCP/853 | TLSでDNS通信を暗号化。専用ポートのためブロック可能 |
| DoH(RFC 8484) | TCP/443 | HTTPS上でDNS通信。通常のHTTPS通信と区別困難 |

### オープンリゾルバ(Open Resolver)

オープンリゾルバとは、**インターネット上の任意のホストからの再帰的DNSクエリを受け付けるDNSキャッシュサーバ**のことです。

#### 問題点

- **DNS Amplification攻撃の踏み台**として悪用される
- **DNSキャッシュポイズニングの対象**になる
- DNS水責め攻撃の中継点となる

#### 対策

- 再帰的問い合わせを**内部ネットワークのクライアントのみに制限**する
- ACL(アクセス制御リスト)で許可するIPアドレス範囲を設定
- BIND設定例：`allow-recursion { 内部ネットワーク; };`

**IPA試験では「踏み台にならないための対策」として頻出のテーマです。**

### TSIG(Transaction SIGnature)

TSIG(RFC 2845)とは、**DNSサーバ間の通信(ゾーン転送や動的更新)を共有秘密鍵(HMAC)で認証する仕組み**です。

#### 特徴

- DNSメッセージに**HMAC-MD5/SHA256等の署名**を付加
- **ゾーン転送(AXFR)の認証**に主に使用される
- DNSSECとは異なり、**サーバ間の認証**が目的

#### TSIGとDNSSECの違い

| 項目 | TSIG | DNSSEC |
|------|------|--------|
| 目的 | サーバ間認証 | 応答の真正性・完全性 |
| 鍵方式 | 共通鍵(HMAC) | 公開鍵暗号 |
| 主な用途 | ゾーン転送、動的更新 | クライアントへの応答検証 |
| 対象 | DNSサーバ間 | リゾルバ～権威DNS間 |
...
| 種類 | 説明 |
|------|------|
| AXFR(Full Zone Transfer) | ゾーン全体を転送 |
| IXFR(Incremental Zone Transfer) | 差分のみを転送 |

#### セキュリティリスク

- 外部からのAXFRリクエストに応答すると、**ゾーン内の全レコード(内部ホスト名、IPアドレス等)が漏洩**
- 攻撃者が内部ネットワーク構成を把握し、次の攻撃に利用可能

#### 対策

- ゾーン転送を許可するIPアドレスの制限(`allow-transfer`)
- TSIGによる認証
- ファイアウォールでTCP/53の外部アクセスを制限

### DNSトンネリング(DNS Tunneling)

DNSトンネリングとは、**DNSクエリとレスポンスにデータを埋め込むことで、DNSプロトコルを通信チャネルとして悪用し、ファイアウォールを迂回してデータを送受信する手法**です。

#### 仕組み

1. マルウェアがデータを**Base32/Base64エンコード**しサブドメイン名に埋め込む
   例：`aGVsbG8.evil.example.com`
2. 攻撃者が管理する権威DNSサーバが応答にデータを埋め込んで返す
3. DNS通信を許可しているファイアウォールを通過可能

#### 用途(攻撃者視点)

- C2(Command and Control)通信
- 機密データの持ち出し(Data Exfiltration)
- マルウェアのダウンロード

#### 検知・対策

- DNSクエリの**異常に長いサブドメイン名**を検知
- DNSクエリの**頻度・量の監視**
- 特定ドメインへの大量クエリの検出
- DNS通信をプロキシ経由に制限し、直接のDNS通信をブロック
...
| 攻撃名 | 主目的 | 対象 | キーワード |
|------|------|------|------|
| DNSキャッシュポイズニング | 偽サイト誘導 | キャッシュDNS | トランザクションID偽装 |
| カミンスキー攻撃 | 偽サイト誘導(高度版) | キャッシュDNS | ランダムサブドメイン+NS汚染 |
| DNS Amplification | DDoS(帯域枯渇) | 攻撃対象サーバ | 増幅+IPスプーフィング |
| DNS水責め | DoS(権威DNS過負荷) | 権威DNS | ランダムサブドメイン+キャッシュ不成立 |
| DNSトンネリング | データ持ち出し/C2通信 | ファイアウォール迂回 | サブドメインにデータ埋め込み |
| ゾーン転送攻撃 | 情報収集 | 権威DNS | AXFR不正利用 |
