...
SQLインジェクションとは、**Webアプリケーションの入力値がSQL文に直接埋め込まれる際に、攻撃者が意図的にSQL構文を注入し、データベースを不正に操作する攻撃**です。

**OWASP Top 10で常に上位に位置し、情報処理安全確保支援士試験で最も頻出の攻撃の一つです。**
...
攻撃者が入力値に `' OR '1'='1` を挿入すると：

```sql
SELECT * FROM users WHERE id = '' OR '1'='1'
```

条件が常にTRUEとなり、**全レコードが返却**されます。

### SQLインジェクションの種類

| 種類 | 説明 |
|------|------|
| インバンドSQLインジェクション | エラーメッセージやレスポンスに直接結果が表示される(Error-based / Union-based) |
| ブラインドSQLインジェクション | レスポンスに直接結果が表示されず、True/False の挙動差や応答時間差で情報を推測(Boolean-based / Time-based) |
| セカンドオーダーSQLインジェクション | 入力時ではなく、**保存されたデータが別のSQL文で使用される際に発動**(蓄積型) |

#### ブラインドSQLインジェクション(試験で頻出)

レスポンスにSQL結果が直接表示されない場合でも、以下の手法で情報を抽出可能です。

- **Boolean-based**：条件のTrue/Falseでページ表示が変わることを利用
- **Time-based**：`WAITFOR DELAY` や `SLEEP()` を使い、応答時間差で真偽を判定

### SQLインジェクションによる被害

- **データベースの全データ漏洩**(個人情報、認証情報等)
- **データの改ざん・削除**(UPDATE/DELETE文の注入)
- **認証回避**(ログインバイパス)
- **OSコマンド実行**(DBMSの機能を利用：`xp_cmdshell`等)
- **データベースサーバの完全掌握**

### SQLインジェクション対策

#### プリペアドステートメント(バインド機構)【最重要】

**SQL文とパラメータを分離し、入力値をSQL構文として解釈させない**方式です。
...
- **SQLインジェクションに対する最も効果的かつ根本的な対策**
- パラメータは値としてのみ処理され、SQL構文として解釈されない
- 試験では「バインド機構」「プレースホルダ」として出題される

#### ストアドプロシージャの利用

事前定義されたSQL文のみを実行させる方式です。ただし、ストアドプロシージャ内で動的SQLを組み立てている場合は脆弱性が残ります。

#### エスケープ処理

シングルクォート `'` を `''` にエスケープする等の処理です。**バインド機構が使えない場合の補助的対策**であり、単独での使用は推奨されません。

#### 最小権限の原則

- アプリケーションが使用するDBアカウントに**必要最小限の権限のみ付与**
- DROPやGRANTなどの管理権限を与えない

#### エラーメッセージの制御

- SQLエラーの詳細をユーザに表示しない(テーブル名、カラム名の漏洩防止)
- カスタムエラーページを使用

#### WAF(Web Application Firewall)

SQL構文を含むリクエストを検知・遮断する補助的対策です。

## OSコマンドインジェクション(OS Command Injection)

### 定義

OSコマンドインジェクションとは、**Webアプリケーションがユーザ入力をOSコマンドの一部として使用する際に、攻撃者がOS上の任意のコマンドを実行させる攻撃**です。
...
攻撃者が入力値に `; cat /etc/passwd` を挿入すると：

```bash
ping ; cat /etc/passwd
```

`ping` の後に **任意のOSコマンドが実行**されます。

### コマンド連結に使用される文字

| 文字 | 意味 |
|------|------|
| `;` | コマンドの区切り(前のコマンドの成否に関係なく実行) |
| `&&` | 前のコマンドが成功した場合に次を実行 |
| `\|\|` | 前のコマンドが失敗した場合に次を実行 |
| `\|` | パイプ(前のコマンドの出力を次の入力に) |
| `` ` `` / `$()` | コマンド置換 |
...
- **OSコマンドを呼び出さない実装**にする(ライブラリ/APIの使用)
- やむを得ない場合は**入力値のホワイトリスト検証**(許可する文字のみ)
- シェル呼び出しを回避する関数の使用(例：Pythonの`subprocess.run(["ping", input], shell=False)`)
- 実行権限の最小化

## ディレクトリトラバーサル(Directory Traversal / Path Traversal)

### 定義

ディレクトリトラバーサルとは、**ファイルパスを指定するパラメータに `../` 等の相対パスを挿入し、本来アクセスできないサーバ上のファイルを読み取る攻撃**です。
...
`../` を繰り返すことでWebアプリケーションの公開ディレクトリの外にあるファイルにアクセスします。

### 被害

- 設定ファイルの読取(`/etc/passwd`、`web.xml`、`.env`等)
- ソースコードの漏洩
- 認証情報の窃取
- ログファイルの不正閲覧
...
- **ファイル名を直接パラメータにしない**(IDで管理し、サーバ側でパスを解決)
- 入力値から `../` やパス区切り文字を除去・正規化
- **chroot / サンドボックス**によるアクセス範囲の制限
- Webアプリケーションの実行ユーザに最小権限を付与

## HTTPヘッダインジェクション(HTTP Header Injection)

### 定義

HTTPヘッダインジェクションとは、**HTTPレスポンスヘッダにユーザ入力が反映される際に、改行コード(CRLF: `\r\n`)を挿入して任意のヘッダやボディを注入する攻撃**です。
...
```
Location: https://example.com/page?redirect=<user_input>
```

攻撃者が `\r\nSet-Cookie: SESSIONID=evil` を挿入すると：

```
Location: https://example.com/page?redirect=xxx
Set-Cookie: SESSIONID=evil
```

**任意のHTTPヘッダが注入**されます。

### HTTPレスポンス分割(HTTP Response Splitting)

CRLFを2つ連続で挿入(`\r\n\r\n`)すると、レスポンスボディの注入が可能になり、**偽のHTMLページを表示させる(XSSと同等の被害)**ことができます。

### 被害

- セッション固定化(Set-Cookie注入)
- キャッシュポイズニング
- XSSと同等の攻撃(レスポンスボディ注入)
- フィッシングページの表示

### 対策

- HTTPヘッダに出力する値から**改行コード(CR/LF)を除去**
- URLエンコード後の検証
- フレームワークの安全なリダイレクト関数を使用

## メールヘッダインジェクション

### 定義

問い合わせフォーム等でメール送信機能がある場合に、**宛先(To)や件名(Subject)フィールドに改行コードを挿入し、任意のメールヘッダを追加して迷惑メールやフィッシングメールを送信させる攻撃**です。

### 対策

- メールヘッダに出力する値から改行コードを除去
- 宛先アドレスを固定値にする(ユーザ入力を宛先にしない)
- メール送信ライブラリの安全な関数を使用
...
SSRFとは、**攻撃者がサーバ側のアプリケーションに対して、意図しないリクエストを内部ネットワークや外部サービスに送信させる攻撃**です。

### 攻撃の仕組み

Webアプリケーションがユーザ指定のURLからデータを取得する機能を持つ場合：

```
https://example.com/fetch?url=http://internal-server/admin
```

攻撃者がURLパラメータに内部ネットワークのアドレスを指定すると：

- **ファイアウォールの内側にある内部サーバへのアクセス**が可能になる
- サーバが「正規のリクエスト元」として通信するため、アクセス制御をバイパスできる

### 典型的な攻撃対象

| 対象 | 例 |
|------|------|
| 内部ネットワーク | `http://192.168.1.1/admin`、`http://localhost:8080` |
| クラウドメタデータ | `http://169.254.169.254/latest/meta-data/`(AWSのIAM認証情報取得) |
| 内部サービス | データベース管理画面、監視ダッシュボード |

**AWSなどクラウド環境では、メタデータAPIからIAMクレデンシャルが取得される被害が多発しています。**

### 被害

- 内部ネットワークの情報収集(ポートスキャン等)
- 内部サービスへの不正アクセス
- クラウド環境の認証情報窃取
- 他システムへの攻撃の踏み台
...
- ユーザ入力をURLとして使用しない設計
- URL のホワイトリスト制御(許可するドメイン/IPのみ)
- プライベートIPアドレス範囲(RFC 1918)やループバックアドレスへのリクエストを禁止
- クラウド環境でのメタデータAPIへのアクセス制限(IMDSv2の利用等)
- DNSリバインディング対策
...
オープンリダイレクトとは、**Webアプリケーションのリダイレクト先URLがユーザ入力に依存しており、攻撃者が任意のURLへリダイレクトさせられる脆弱性**です。

### 攻撃例

```
https://example.com/login?redirect=https://evil.example.com/phishing
```

正規サイトのURLに見えるため、ユーザは安心してクリックしてしまいます。

### 被害

- フィッシングサイトへの誘導(正規ドメインのURLを利用するため信用されやすい)
- 認証情報の窃取
- マルウェア配布サイトへの誘導
- OAuthフローの悪用(認可コードの窃取)

### 対策

- リダイレクト先を**ホワイトリストで制限**(許可するURLのみ)
- 外部URLへのリダイレクトを禁止(相対パスのみ許可)
- リダイレクトURLにトークンを付与して検証
- ユーザにリダイレクト先を表示し確認させる
...
## 同一オリジンポリシー(Same-Origin Policy)とCORS

### 同一オリジンポリシー(SOP)とは

同一オリジンポリシーとは、**Webブラウザが実装するセキュリティ機構で、あるオリジン(スキーム+ホスト+ポート)から読み込まれたスクリプトが、別のオリジンのリソースにアクセスすることを制限する**仕組みです。

### オリジンの定義

オリジンは以下の3要素の組み合わせで決まります。
...
**3要素がすべて一致する場合のみ「同一オリジン」**です。

例：

| URL | `https://example.com/page` との関係 |
|------|------|
| `https://example.com/other` | 同一オリジン |
| `http://example.com/page` | **異なる**(スキームが異なる) |
| `https://sub.example.com/page` | **異なる**(ホストが異なる) |
| `https://example.com:8080/page` | **異なる**(ポートが異なる) |

### SOPが防ぐ攻撃

- 悪意のあるサイトが、ユーザがログイン中の別サイトのデータを読み取ること
- XSSによる異なるオリジンの情報窃取の一部

### SOPの制限を受けないもの

以下はSOPの制限を受けずにクロスオリジンで利用可能です。

- `<img>` タグによる画像読み込み
- `<script>` タグによるJavaScript読み込み
- `<link>` タグによるCSS読み込み
- `<form>` タグによるフォーム送信(**CSRFが成立する理由**)

### CORS(Cross-Origin Resource Sharing)

CORSとは、**SOPによる制限を安全に緩和するための仕組み**です。サーバがHTTPレスポンスヘッダで許可するオリジンを指定します。
...
| `Access-Control-Allow-Headers` | 許可するリクエストヘッダを指定 |
| `Access-Control-Allow-Credentials` | Cookie等の認証情報の送信を許可 |

#### プリフライトリクエスト(Preflight Request)

単純でないリクエスト(PUT/DELETEメソッドやカスタムヘッダ等)の前に、ブラウザが自動的に **OPTIONSメソッドで事前確認**を行います。サーバが許可を返した場合のみ、本来のリクエストが送信されます。
...
上記のように**ワイルドカード(`*`)と認証情報の許可を同時に設定**すると、任意のサイトからの認証付きリクエストが通ってしまいます(ブラウザはこの組み合わせを拒否するが、設定意図としては危険)。

**CORSの設定不備は午後問題で問われやすいポイントです。**

## WAF(Web Application Firewall)

### 定義

WAF(Web Application Firewall)とは、**Webアプリケーションへの通信を検査し、SQLインジェクション、XSS、OSコマンドインジェクション等の攻撃パターンを検知・遮断するセキュリティ装置**です。

### 従来のファイアウォール/IPSとの違い

| 装置 | 検査レベル | 主な対象 |
|------|------|------|
| FW(ファイアウォール) | L3/L4(IP/ポート) | 不正なIP/ポートへの通信 |
| IPS(侵入防止システム) | L3〜L7(シグネチャ) | 既知の攻撃パターン |
| **WAF** | **L7(HTTPの内容)** | **Webアプリケーション攻撃** |

### WAFの検知方式

| 方式 | 説明 |
|------|------|
| シグネチャベース(ブラックリスト) | 既知の攻撃パターンと照合。未知の攻撃には対応困難 |
| ホワイトリスト | 正常な通信パターンのみ許可。誤検知(False Positive)の調整が必要 |
| アノマリベース(異常検知) | 通常の通信パターンと異なる通信を検知 |

### WAFの導入形態

| 形態 | 特徴 |
|------|------|
| リバースプロキシ型 | Webサーバの前段に設置。最も一般的 |
| インライン(ブリッジ)型 | ネットワーク経路上に透過的に配置 |
| クラウドWAF | CDNと一体化。導入が容易(Cloudflare、AWS WAF等) |

### WAFの限界

- **アプリケーションの脆弱性を根本的に修正するものではない**(あくまで補助的対策)
- 誤検知(False Positive)により正常通信を遮断するリスク
- 暗号化通信(HTTPS)を検査するにはSSL終端が必要
- ゼロデイ攻撃への対応は限定的

**WAFは「保険的対策」であり、プリペアドステートメントやエスケープ処理等の根本対策と併用する必要があります。**
...
| 攻撃 | 注入先 | 主な被害 | 根本対策 |
|------|------|------|------|
| SQLインジェクション | SQL文 | DB漏洩/改ざん | プリペアドステートメント |
| XSS | HTML/JavaScript | セッション窃取/フィッシング | 出力時エスケープ |
| OSコマンドインジェクション | OSコマンド | サーバ完全掌握 | OSコマンド不使用 |
| HTTPヘッダインジェクション | HTTPレスポンスヘッダ | セッション固定/XSS | 改行コード除去 |
| メールヘッダインジェクション | メールヘッダ | スパム送信 | 改行コード除去 |
| ディレクトリトラバーサル | ファイルパス | ファイル漏洩 | パス直接指定の回避 |

### 攻撃の実行場所による分類

| 分類 | 攻撃 | 攻撃が実行される場所 |
|------|------|------|
| クライアント側攻撃 | XSS、CSRF、クリックジャッキング | 利用者のブラウザ |
| サーバ側攻撃 | SQLi、OSコマンドi、SSRF、ディレクトリトラバーサル | Webサーバ/DB |
| 通信経路攻撃 | MITM、SSL Stripping、ダウングレード | ネットワーク経路 |
| ブラウザ内部攻撃 | MITB | マルウェア感染ブラウザ |
